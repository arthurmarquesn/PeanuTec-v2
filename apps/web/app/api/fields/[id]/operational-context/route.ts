...
import { getFieldOperationalContext } from "@/server/operational/field-operational.service";
...
export async function GET(_request: NextRequest, context: RouteContext) {
  try {
    ...
    return NextResponse.json(await getFieldOperationalContext(id));
  } catch (error: unknown) {
    ...
    console.error("[GET /api/fields/[id]/operational-context]", error);
    return NextResponse.json({ detail: "Não foi possível carregar o contexto operacional do talhão." }, { status: 500 });
  }
}
