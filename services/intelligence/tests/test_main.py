...
def _hourly_weather() -> dict:
    ...
    return {
        "time": [item.strftime("%Y-%m-%dT%H:%M") for item in times],
...
def _historical_rainfall() -> dict:
    today = date.today()
    days = [today - timedelta(days=40 - index) for index in range(40)]
    return {
...
def _payload(
    ...
    return {
        "field": {
            ...
            "latitude": -21.9347,
            "longitude": -50.5136,
            "data_plantio": planting_date or (date.today() - timedelta(days=90)).isoformat(),
            "cultura": "Amendoim",
            "doenca_alvo": disease,
...
def _assert_compact_response(body: dict, disease_prefix: str) -> None:
    assert body["field"]["name"] == "Talhao Teste"
    assert body["disease"].startswith(disease_prefix)
    assert isinstance(body["actions"], list)
    assert set(body["risk"]) == {"climate_index", "agronomic_index", "classification"}
...
def test_black_spot_valid_snapshot() -> None:
    response = client.post("/analisar", json=_payload("Mancha-preta"))
    assert response.status_code == 200
    _assert_compact_response(response.json(), "Mancha-preta")


def test_brown_spot_valid_snapshot() -> None:
    response = client.post("/analisar", json=_payload("Mancha-castanha"))
    assert response.status_code == 200
    _assert_compact_response(response.json(), "Mancha-castanha")


def test_invalid_disease() -> None:
    response = client.post("/analisar", json=_payload("Ferrugem"))
...
def test_future_planting_date() -> None:
    response = client.post(
        "/analisar",
        json=_payload(planting_date=(date.today() + timedelta(days=1)).isoformat()),
    )
    assert response.status_code == 422
...
def test_snapshot_without_precipitation_sum() -> None:
    payload = _payload()
    del payload["weather"]["historical_daily_rainfall"]["precipitation_sum"]
    response = client.post("/analisar", json=payload)
...
def test_field_out_of_field_status() -> None:
    response = client.post("/analisar", json=_payload(status="arrancado"))
    assert response.status_code == 200
    assert response.json()["management_relevance"]["status"] in {
